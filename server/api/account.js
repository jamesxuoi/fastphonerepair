...
router.get('/user-data/:accountid', async (req, res) => {
    ...
    // Allow users to access their own data regardless of authLevel
    if (req.session.user.accountid !== accountid && req.session.user.authLevel < 2) {
    ...
    const userSql = `SELECT u.id,u.email,u.fullname FROM users u WHERE u.id = ?`
    ...
    connection.query({ sql: userSql, values: [accountid] }, (error, userResults) => {
        if (error) {
        ...
        const userData = userResults[0] // Fixed: Properly declare userData
        console.log('✅ User data found:', userData)

        // Second query: Get tickets data
        connection.query({ sql: ticketsSql, values: [accountid] }, (error, ticketResults) => {
            if (error) {
                console.error('❌ Tickets query error:', error)
                return res.status(400).json({ error: 'Database error' })
            }
            
            const ticketData = ticketResults || []
            console.log('✅ Ticket data found:', ticketData.length, 'tickets')
            
            const responseData = { ticketData, userData }
